...
const getGenreString = (detail) => {
    if (!detail) return '';
    const { genres, first_air_date } = detail;
    let text = genres?.map(genre => genre.name).join(', ') || '';
    if (first_air_date) text += ` • ${first_air_date.split('-')[0]}`;
    return text;
};

const TVPoster = ({ imageUrl, backdropUrl }) => (
...
export default function SingleTVPage({ route }) {
  ...
  const backdropUrl = baseUrl + tvShow.backdrop_path;
  ...
  return (
    <ScrollView style={styles.container}>
      <TVPoster imageUrl={imageUrl} backdropUrl={backdropUrl} />
