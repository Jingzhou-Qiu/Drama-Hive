...
const TVShowPre = ({ tvShow }) => {
    ...
    const fetchImage = useCallback(async () => {
        try {
            const response = await fetch(
                `https://api.themoviedb.org/3/tv/${tvShow.id}/images`,
                {
                    method: 'GET',
                    headers: {
    ...
    return (
        <TouchableOpacity style={styles.container} onPress={handleNavigation}>
            ...
            <Text style={styles.rating}>Rating: {tvShow.vote_average.toFixed(1)}</Text>
            <Text style={styles.overview}>Overview: {formatOverview(tvShow.overview || "")}</Text>
        </TouchableOpacity>
    );
};
